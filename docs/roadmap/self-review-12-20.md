...
|---|---|---|---|---|
| R-01 | 🔴 High | `serve.py` | Under `gunicorn` — the documented production command — nothing drains the review worker on `SIGTERM`. The drain is wired only into the `wsgiref` entry point. | ✅ closed |
