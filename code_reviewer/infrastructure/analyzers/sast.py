...
class SASTAnalyzer:
    ...
    def _compile_patterns(self):
        ...
        for vuln_type, patterns in self.PYTHON_PATTERNS.items():
            self._compiled_patterns["python"][vuln_type] = [
                (re.compile(p[0], re.IGNORECASE), p[1], p[2], p[3], p[4]) for p in patterns
            ]
    ...
    def _generate_summary(self, report: SASTReport) -> str:
        ...
        parts.append(
            f"{risk_emoji.get(risk.risk_level, '')} **Security Risk Level**: {risk.risk_level.upper()}"
        )
